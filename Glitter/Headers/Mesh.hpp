...
public:
    ...
    MeshData(const std::vector<Vertex>& v,
...
public:
    ...
    MeshBuffer(const MeshData& data) {
        setupMesh(data);
    }
...
class MeshRenderer {
public:
    void Draw(const MeshData& data, const MeshBuffer& buffer, Shader& shader) {
        unsigned int diffuseNr = 1;
        unsigned int specularNr = 1;
        unsigned int normalNr = 1;
        unsigned int heightNr = 1;
        ...
        for (unsigned int i = 0; i < data.textures.size(); i++) {
            ...
            std::string number;
            std::string name = data.textures[i].type;
        ...
        buffer.Bind();
        glDrawElements(GL_TRIANGLES, static_cast<unsigned int>(data.indices.size()), GL_UNSIGNED_INT, 0);
        buffer.Unbind();
